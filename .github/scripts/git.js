...
// All tags, one per line -> array. Includes the legacy rolling prerelease tags
// (even-latest/mobile-latest); parseTag ignores anything non-semver.
function allTags() {
  const out = git(["tag", "-l"]);
...
function previousReleaseTag(versionModule) {
  ...
  for (const t of allTags()) {
    ...
    if (!p) continue; // ignores non-semver tags
...
function fetchManifest(tag) {
  if (!tag) return null;
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "tenir-manifest-"));
  try {
