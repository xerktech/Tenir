...
// Component -> display heading. Web changes fold into the API heading (the SPA
// is baked into the api image); an unmatched change (empty components) lands in
// "Other".
...
const HEADING_ORDER = ["API", "Parakeet STT", "Nemotron STT", "Even", "Mobile", "Other"];
...
const TABLE_ORDER = ["api", "parakeet-stt", "nemotron-stt", "even", "mobile"];
...
function renderReleaseNotes(opts) {
  ...
  return [`## Tenir v${version}`, "", renderComponentTable(manifest), "", body, "", INSTALL_DETAILS, ""].join("\n");
}
